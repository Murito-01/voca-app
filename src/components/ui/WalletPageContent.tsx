...
export default function WalletPageContent() {
  ...
  const { balance, locked_balance, stats, transactions } = data
  ...
  return (
    <section className="mx-auto w-full max-w-3xl">
      ...
      {/* Withdraw Funds Button + Coming Soon Banner */}
      {showWithdrawBanner && (
        <div className="mb-3 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          ...
          <div>
            <p className="font-semibold">Fitur Penarikan Segera Hadir</p>
            <p className="text-xs mt-0.5 text-amber-700">
              Integrasi payment gateway sedang dalam pengembangan. Kamu akan bisa menarik saldo ke rekening bank atau e-wallet segera.
            </p>
          </div>
          <button
            onClick={() => setShowWithdrawBanner(false)}
            className="ml-auto shrink-0 text-amber-500 hover:text-amber-700 transition-colors text-base"
            aria-label="Tutup"
          >
            ×
          </button>
      ...
      <button
        ...
        className="w-full py-3 rounded-xl border-2 border-dashed border-gray-300 text-sm font-semibold text-gray-500 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50/50 transition-all active:scale-[0.99] flex items-center justify-center gap-2"
      >
        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
        </svg>
        Withdraw Funds
        <span className="ml-1 rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-bold text-gray-400 uppercase tracking-wider">Soon</span>
      </button>
    </section>
  )
}
